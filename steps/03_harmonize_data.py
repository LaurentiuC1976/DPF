...
def main(df):
    airport_list = json.loads(
        SnowflakeFile.open("@raw.raw/airport_list.json", 'r', require_scoped_url = False).read()
    )
...
pipeline = [
    ...
    View(
        ...
        where departure_airport = (
            select $1:airport 
            from @git.public.git_repo/branches/main/data/home.json 
...
for view in pipeline:
    public_schema.views.create(view, mode=CreateMode.or_replace)
